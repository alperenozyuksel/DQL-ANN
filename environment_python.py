    ...
    def run(self):
        ...
        while running:
            ...
            if hits:
                ...
                # Hata sayacını ve ceza birikimini sıfırlıyoruz
                self.miss_count = 0
                self.penalty_accumulator = 0
                print("Total reward: ", self.total_reward)

            elif self.m1.rect.top > HEIGHT:
                # Düşman kaçarsa ceza artırılıyor
                self.miss_count += 1  # Hataları sayıyoruz
                self.penalty_accumulator += 50  # Cezayı artırıyoruz (artış miktarı ayarlanabilir)

                # İlk temel ceza üzerine birikimli cezayı ekliyoruz
                self.reward = -150 - self.penalty_accumulator
                self.total_reward += self.reward

                self.done = True
                running = False
                print(
                    f"Kutuyu kaçırdın! Hata sayısı: {self.miss_count}, Toplam ceza: {self.penalty_accumulator}, Total reward: {self.total_reward}")
